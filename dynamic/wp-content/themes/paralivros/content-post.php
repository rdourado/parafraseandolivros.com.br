...
<?php 				if ( in_category( 'resenha' ) ) get_template_part( 'acf', 'resenha' ); ?>
					<?php the_content( '<span>Leia mais…</span>' ) ?>
<?php 				if ( in_category( 'lancamentos' ) ) get_template_part( 'acf', 'lancamentos' ); ?>
					<?php if ( function_exists( 'st_add_widget' ) ) echo st_add_widget( '' ); ?>
				</div>
			</article>
